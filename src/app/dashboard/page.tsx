    ...
    const fetchNftData = async () => {
      ...
      try {
        const response = await fetch(
          `https://yearning-nelie-shagbaortechnology-e5aa7049.koyeb.app/api/user-nfts/?wallet_address=${bech32Address}`
        )
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 lg:gap-5 xl:gap-8 rounded-xl border sm:border-white md;border-none overflow-x-auto p-5">
          {nfts.map((nft, index) => (
            ...
            >
              <Image src={`${nft.imageUrl}`} alt={`${nft.params.name}`} width={300} height={300} unoptimized={true}
 className="rounded-lg object-contain h-20 md:h-40 w-full" />
